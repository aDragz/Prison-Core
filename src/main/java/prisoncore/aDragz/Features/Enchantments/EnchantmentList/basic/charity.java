...
public class charity {
    ...
    private static int maxValue = cfgYaml.getInt("Charity.Max-Value")+1; //+1 or it'll cancel out at 999
    
    private static long defaultValue = cfgYaml.getLong("Charity.Value");
    private static long divide = cfgYaml.getLong("Charity.Divide");
    
    public static void procCharity(int level, Player player) {
        
        //100000 * (level/10)
        long value = (defaultValue * (level/divide))+defaultValue;
        
        Bukkit.getOnlinePlayers().forEach(S -> {
            long beforeTokens = storeTokens.tokens.get(S.getUniqueId().toString());
            long afterTokens = (beforeTokens + value);
            storeTokens.tokens.put(S.getUniqueId().toString(), afterTokens);
            
            S.sendMessage(grabMessagesValue.type("enchants", "charity_give").replaceAll("&", "§").replaceAll("TOKENS", String.valueOf(value)).replaceAll("MONEY", String.valueOf(value*15)));
            
            //Give Money
            EconomyResponse r = main.econ.depositPlayer(S.getPlayer(), value*15);
        });
    }
    
    //Pickaxe Lore:
    public static boolean addCharityEnchant(Player player, int value, int oldAmountToAdd, ItemStack pickaxe) {
        if (value < maxValue) {
            try {
                 ...
                 */
                ArrayList<String> pickLore = (ArrayList<String>) Objects.requireNonNull(updateLore.updatePickaxeMeta(pickaxe.getItemMeta(), "Charity", value, oldAmountToAdd)); //Adds lore to pickaxe
